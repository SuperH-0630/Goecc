...
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEUTzVCpSLjZ8YnYSys6nvCFEIqTVY
d1eL5llAwA1jaqtYPlouHWQQp2AwE5T6TRIX1Qyz6GJuGgKut3IvpaykKA==
-----END PUBLIC KEY-----`
	pemPriKey = `-----BEGIN WUNCHAIN PRIVATE KEY-----
MHcCAQEEIEJS9RWEZ9kR7GIh6ytyx+KAeYtGb2bIWXlmEUMDEmN6oAoGCCqGSM49
AwEHoUQDQgAEUTzVCpSLjZ8YnYSys6nvCFEIqTVYd1eL5llAwA1jaqtYPlouHWQQ
p2AwE5T6TRIX1Qyz6GJuGgKut3IvpaykKA==
-----END WUNCHAIN PRIVATE KEY-----`
...
func TestEccEncryptBase64(t *testing.T) {
	...
	assert.Nil(t, err)
	_, err = EccEncrypt([]byte(msg), base64PriKey)
	assert.NotNil(t, err)
...
func TestEccSign(t *testing.T) {
	...
	assert.Nil(t, err)
	_, _, err = EccSign([]byte(msg), pemKey.PublicKey)
	assert.NotNil(t, err)
	_, _, err = EccSign([]byte(msg), pemPubKey)
	assert.NotNil(t, err)

	res := EccVerifySign([]byte(msg), rText, sText, pemKey.PublicKey)
	assert.Equal(t, res, true)

	res = EccVerifySign([]byte(msg), rText, sText, pemKey.PrivateKey)
	assert.Equal(t, res, false)
	res = EccVerifySign([]byte(msg), sText, rText, pemKey.PrivateKey)
	assert.Equal(t, res, false)
}
